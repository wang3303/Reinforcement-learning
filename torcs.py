...
def playgame(train = 1):
	...
	for i in xrange(epoch):
		...
		for j in xrange(100000):#max step in a epoch is set to 100000
			if epsilon > 0.01:#decreasing randomness of action selection
				epsilon -= 1e-4
			action = np.zeros((1,3))
			action[0][1] += 0.6
			action[0][2] -= 1
			if random.random() < epsilon:
				action[0][0] += np.random.normal(0,0.3)
				action[0][1] += np.random.normal(0,0.4)
				action[0][2] += np.random.normal(0,0.05)
			else:
				action = net.predict(state.reshape(1,-1))#action shape should be (1,3)
			#action = np.array([0,1,-1]) for testing
			...
			for replay in minibatch:
				batch_state,action,r,batch_new_state,done = replay
			...
			if train:
				...
				grad = sess.run(tf.gradients(critic.output, critic_action), feed_dict = {
